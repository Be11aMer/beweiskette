...
import { formatDate, truncateHash, sanitizeText, formatFileSize } from '../utils.js';
      ...
      <div class="chain-entry ${entryClass}">
        <div class="card">
          <div class="entry-meta">
            <span class="entry-filename">${sanitizeText(entry.evidence.file_name)}</span>
          ...
          <div class="entry-details">
            <div>
              ...
              <div class="entry-detail-value">${sanitizeText(entry.custody.custodian)}</div>
            </div>
            <div>
              <div class="entry-detail-label">File Size</div>
              <div class="entry-detail-value">${formatFileSize(entry.evidence.file_size)}</div>
            </div>
            ${entry.custody.case_reference ? `
              <div>
                <div class="entry-detail-label">Case Ref</div>
                <div class="entry-detail-value">${sanitizeText(entry.custody.case_reference)}</div>
              </div>
            ` : ''}
            <div>
              <div class="entry-detail-label">File Type</div>
              <div class="entry-detail-value">${sanitizeText(entry.evidence.file_type)}</div>
            </div>
            ${exifFields.map(([label, value]) => `
              <div>
                <div class="entry-detail-label">${label}</div>
                <div class="entry-detail-value">${sanitizeText(String(value))}</div>
              </div>
            `).join('')}
            ${entry.custody.notes ? `
              <div class="entry-notes">"${sanitizeText(entry.custody.notes)}"</div>
            ` : ''}
          </div>
        </div>
      </div>
  ...
  container.querySelectorAll('.entry-hash').forEach(el => {
    el.addEventListener('click', () => {
      ...
      navigator.clipboard.writeText(hash).then(() => {
        const original = el.querySelector('.copy-btn').innerHTML;
        el.querySelector('.copy-btn').innerHTML = `
          <svg viewBox="0 0 24 24" fill="none" stroke="var(--success)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M20 6L9 17l-5-5"/>
          </svg>
        `;
        setTimeout(() => {
          el.querySelector('.copy-btn').innerHTML = original;
        }, 1500);
      });
    });
  });
